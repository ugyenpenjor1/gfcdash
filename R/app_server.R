...
# )
  ...
  #
  # GIF:
  #   renderImage() sends the existing GIF file to the browser.
  #
  # HTML:
  #   The HTML animation is copied to a temporary preview directory and
  #   served using Shiny's resource system. This directory contains the
  #   HTML file plus all of the JS, CSS and image files generated by
  #   animation::saveHTML().
  # ---------------------------------------------------------------------------
  output$animation_display <- renderUI({
    ...
    if (!file.exists(rv$animation_path)) {
      return(
        div(
    ...
    if (rv$animation_type == "gif") {
      ...
      tagList(
        ...
        div(
          ...
          "The GIF animation is displayed above. ",
      ...
      # HTML preview
      #
      # Create a session-specific preview directory containing the complete
      # HTML animation folder structure.
      # -----------------------------------------------------------------------
      ...
      # Copy the complete animation directory into the preview directory.
      #
      # saveHTML() produces:
      #
      #   gfc_animation.html
      ...
      #   css/
      #
      # All of these are required by the HTML animation.
      #
      animation_files <- list.files(
        dirname(rv$animation_path),
        all.files = TRUE,
        full.names = TRUE,
        recursive = FALSE,
        no.. = TRUE
      ...
      # Register the complete preview directory as a Shiny resource.
      preview_prefix <- paste0(
        "gfc_animation_preview_",
        session$token
      )
      
      # Remove an existing registration if this output has been regenerated.
      try(
      ...
      tagList(
        ...
        div(
          ...
          "The HTML animation is displayed above. ",
          "Use the download button below to download the complete ",
          "HTML animation as a ZIP file."
        )
      )
    }
  })
  ...
  output$animation_gif_img <- renderImage({
    ...
    validate(
      need(
        rv$animation_type == "gif",
        "GIF animation not selected."
      )
    )
    
    validate(
      need(
        file.exists(rv$animation_path),
        "Animation file could not be found."
      )
    )
  ...
  #
  # HTML:
  #   Download a ZIP containing the HTML file plus all associated files.
  # ---------------------------------------------------------------------------
  output$download_animation <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (rv$animation_type == "gif") {
        ...
        if (!ok) {
          stop("Could not prepare the GIF for download.")
        }
      ...
      # Create a temporary directory containing the complete HTML animation.
      ...
      # Copy everything generated by saveHTML().
      ...
      # ZIP the complete HTML animation.
      old_wd <- getwd()
